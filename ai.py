...
class ai_agent():
	...
	def operations (self,p_mapinfo,c_control):	
		...
		#-----your ai operation,This code is a random strategy,please design your ai !!-----------------------			
			...
			if myleft <= 2:
				Noleft = True;
			elif (myleft+mywidth) >= (26*16-2):
				Noright = True;
			if mytop <= 2:
				Notop = True;
			elif (mytop+myheight) >= (26*16-2):
				Nodown = True;
			for i in range(0, len(self.mapinfo[2]), 1):
				...
				if ((tile[0][0] in range(myleft-1, myleft+mywidth)) or ((tile[0][0]+tile[0][2]) in range(myleft, myleft+mywidth))) and (tile[1] != 4):
					if (tile[0][1]+tile[0][3]) in range(mytop-3-3, mytop+1):
					#if (tile[0][1]+tile[0][3]) == (mytop-1):
					#	print "No way to Top";
						Notop = True;
						Notop_type = tile[1];
					elif tile[0][1] in range(mytop+myheight-1, mytop+myheight+3+3):
				...
				if ((tile[0][1] in range(mytop-1, mytop+myheight)) or ((tile[0][1]+tile[0][3]) in range(mytop, mytop+myheight))) and (tile[1] != 4):
					if (tile[0][0]+tile[0][2]) in range(myleft-3-3, myleft+1):
					#if (tile[0][0]+tile[0][2]) == (myleft-1):
					#	print "No way to Left";
						Noleft = True;
						Noleft = tile[1];
					elif tile[0][0] in range(myleft+mywidth-1, myleft+mywidth+3+3):
			...
			for j in range(0, len(self.mapinfo[1]), 1):
				...
				if (enemy_bulletL in range(myleft+9-6+1, myleft+14)) and (enemy[0][1] < mytop):
				...
				elif (enemy_bulletL in range(myleft+9-6+1, myleft+14)) and (enemy[0][1] > mytop):
				...
				elif (enemy_bulletT in range(mytop+9-6+1, mytop+14)) and (enemy[0][0] > myleft):
			...
			for i in range(0, len(self.mapinfo[0]), 1):
				...
				if(bullet[0][0] in range(myleft-bullet[0][2], myleft+mywidth)) and (bullet[1] == 0):
					...
					if (bullet[0][1] > mytop) and ((bullet[0][1] - mytop) < (3*16)):
						shoot = 0;
						if (bullet[0][0] in range(myleft-6, myleft+4)) and (not Noleft):
							print "RL"
							move_dir = 3;
						elif (bullet[0][0] in range(myleft+15, myleft+mywidth)) and (not Noright):
							print "RR"
							move_dir = 1;
						elif not Noleft:
							print "RL"
						...
						if (bullet[0][0] in range(myleft+15, myleft+mywidth)) and (not Noleft):
							print "CL";
							move_dir = 3;
							shoot = 0;
						#	self.Update_Strategy(c_control,0,3,1);
						#for j in range(0, bullet[0][0]-myleft-15):
						elif (bullet[0][0] in range(myleft-6, myleft+4)) and (not Noright):
							print "CR";
							move_dir = 1;
							shoot = 0;
						#	self.Update_Strategy(c_control,0,1,1);
						elif (bullet[0][1] > mytop):
							move_dir = 2;
							self.Update_Strategy(c_control,0,move_dir,0);
					...
					if (bullet[0][1] < mytop) and ((mytop - bullet[0][1]) < (3*16)):
						shoot = 0;
						if (bullet[0][0] in range(myleft-6, myleft+4)) and (not Noleft):
							print "RL"
							move_dir = 3;
					...
					if (bullet[0][0] < myleft) and ((myleft - bullet[0][0]) < (3*16)):
						shoot = 0;
						if (bullet[0][1] in range(mytop-6, mytop+4)) and (not Notop):
							print "RU"
							move_dir = 0;
						elif (bullet[0][1] in range(mytop+15, mytop+myheight)) and (not Nodown):
							print "RD"
							move_dir = 2;
						elif not Notop:
							print "RU"
						...
						if (bullet[0][1] in range(mytop+15, mytop+myheight)) and (not Notop):
							print "CU";
							move_dir = 0;
							shoot = 0;
						#	self.Update_Strategy(c_control,0,0,1);
						#for j in range(0, bullet[0][1]-mytop-15):
						elif (bullet[0][1] in range(mytop-6, mytop+4)) and (not Nodown):
							print "CD";
							move_dir = 2;
							shoot = 0;
						#	self.Update_Strategy(c_control,0,2,1);
						#if (bullet[0][0] < myleft):
						elif (bullet[0][0] < myleft):
							move_dir = 3;
							self.Update_Strategy(c_control,0,move_dir,0);
							if (mytop in range(flagtop-myheight, flagtop+flagheight)) and (flagleft < myleft):
								shoot = 0;
					...
					if (bullet[0][0] > myleft) and ((bullet[0][0] - myleft) < (3*16)):
						shoot = 0;
						if (bullet[0][1] in range(mytop-6, mytop+4)) and (not Notop):
							print "RU"
							move_dir = 0;
					...
					#	self.Update_Strategy(c_control,shoot,move_dir,keep_action);
				#else:
			## Trace the nearest/dangerest enemy ##
			if (move_dir == 4):
			#	print ("Min Dist:", min_dist, ", Min Enemy:", min_enemy[0]);
				...
				if (enemyT > mytop) & (not Nodown) & (not (((mytop + myheight) > (enemyT-12)) and (((enemyD == 1) and (enemyL < myleft)) or ((enemyD == 3) and (enemyL > myleft))))):
					print "Trace to Down"
					move_dir = 2;
					shoot = 0;
				elif (enemyL < myleft) & (not Noleft) & (not ((myleft < (enemyL + enemyW+12)) and (((enemyD == 0) and (enemyT > mytop)) or ((enemyD == 2) and (enemyT < mytop))))):
					print "Trace to Left";
					move_dir = 3;
					shoot = 0;
					...
					move_dir = 0;
					shoot = 0;
				elif (enemyT > mytop) & (Nodown_type == 1):
					move_dir = 2;
					if (myleft in range(flagleft-mywidth, flagleft+flagwidth)) and (mytop < (22*16)):
						shoot = 0;
					else:
						shoot = 1;
						print "Shoot D wall"
				...
				elif (enemyL < myleft) & (Noleft_type == 1):
					move_dir = 3;
					#if (mytop in range(flagtop-myheight, flagtop+flagheight)) and (flagleft < myleft):
					if (mytop > (flagtop-(myheight/2))) and (flagleft < myleft):
						shoot = 0;
					else:
						shoot = 1;
						print "Shoot L wall"
					self.Update_Strategy(c_control,0,move_dir,0);
					#self.Update_Strategy(c_control,shoot,move_dir,0);
				elif (enemyL > myleft) & (Noright_type == 1):
					move_dir = 1;
					#if (mytop in range(flagtop-myheight, flagtop+flagheight)) and (flagleft > myleft):
					if (mytop > (flagtop-(myheight/2))) and (flagleft > myleft):
						shoot = 0;
					...
					self.Update_Strategy(c_control,0,move_dir,0);
					#self.Update_Strategy(c_control,shoot,move_dir,0);
				elif (enemyT < mytop) & (Notop_type == 1):
					move_dir = 0;
					shoot = 1;
					print "Shoot T wall"
					self.Update_Strategy(c_control,0,move_dir,0);
			...
			if shoot == 0:
				for enemy in self.mapinfo[1]:
					if enemy[0][0] in range(myleft+9-enemy[0][2]+1, myleft+14):
						if (enemy[0][1]+enemy[0][3]) in range(mytop+1, mytop-3):
							move_dir = 0;
							shoot = 1;
						elif enemy[0][1] in range(mytop+myheight-1, mytop+myheight+3):
							move_dir = 2;
							shoot = 1;
					elif enemy[0][1] in range(mytop+9-enemy[0][3]+1, mytop+14):
						if (enemy[0][0]+enemy[0][2]) in range(myleft+1, myleft-3):
							move_dir = 3;
							shoot = 1;
						elif enemy[0][0] in range(myleft+mywidth-1, myleft+mywidth+3):
							move_dir = 1;
							shoot = 1;
			#shoot = random.randint(0,1)
			#move_dir = random.randint(0,4)
